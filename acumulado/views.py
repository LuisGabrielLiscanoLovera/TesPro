...
def AcumuladoListHistorial(request):
    ...
    acumuladoQsect = ACUMULADO.objects.filter(
        empresa_id=lastEm.lastEm, estatus='I').order_by('-id')
...
def AcumuladoListProc(request):
    ...
    acumuladoProc = ProAcu.objects.filter(
        usuario_id=idUser, empresa_id=lastEm.lastEm, acumulado_id=idAcumulado).order_by('-id')
...
def AcumuladoList(request):
    ...
    lastEm = CambioEmpres.objects.filter(usuario_id=idUser.id).last()
    acumuladoQsect = ACUMULADO.objects.filter(
        empresa_id=lastEm.lastEm, estatus='A').order_by('-id')
    AcumuladoSe = AcumuladoSerializer(acumuladoQsect, many=True)
    dump = json.dumps(AcumuladoSe.data)  # dump serializer to json reponse
...
def AcumuladoListValor(request):
    ...
    lastEm = CambioEmpres.objects.filter(usuario_id=idUser.id).last()
    acumuladoQsect = ACUMULADO.objects.filter(
        empresa_id=lastEm.lastEm).order_by('-id')
    AcumuladoSe = AcumuladoSerializer(acumuladoQsect, many=True)
    dump = json.dumps(AcumuladoSe.data)  # dump serializer to json reponse
...
def createAcumulado(request,):
    ...
    try:
        ...
        data = {'Acumulado': "Acumulado guardado con exito!", 'estatus': True}
...
def AcumuladoDataIntegrante(request):
    ...
    for tareasIntegrante in ProAcu.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm, acumulado_id=int(idAcumulado),
        ...
        tareaIntegrante = (Tarea.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm,
                           id=tareasIntegrante['tarea_id']).values('nom_tarea', 'id')[0])
        totalIntegrante = ProAcu.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm, acumulado_id=int(
        ...
        patinador = Patinador.objects.filter(
            usuario_id=idUser, empresa_id=lastEm.lastEm, id=tareasIntegrante['patinador_id']).distinct().values('integrante_id')
        patinador = Integrante.objects.filter(
            usuario_id=idUser, empresa_id=lastEm.lastEm, id=patinador[0]['integrante_id']).values('nombres', 'apellidos')
        patinador = "{} {}".format(
...
def AcumuladoDataIntegranteValor(request):
    ...
    for tareasIntegrante in ProAcu.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm, acumulado_id=int(idAcumulado),
                                                  integrante_id=idIntegrante).distinct().values('tarea_id', 'patinador_id'):
        tareaIntegrante = (Tarea.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm,
                           id=tareasIntegrante['tarea_id']).values('nom_tarea', 'valor', 'id')[0])
        totalIntegrante = ProAcu.objects.filter(usuario_id=idUser, empresa_id=lastEm.lastEm, acumulado_id=int(
            idAcumulado), integrante_id=idIntegrante, tarea_id=tareaIntegrante['id']).values('tarea_id', 'can_prod_acum').aggregate(can_prod_acum=Sum('can_prod_acum'))
        patinador = Patinador.objects.filter(
            usuario_id=idUser, empresa_id=lastEm.lastEm, id=tareasIntegrante['patinador_id']).distinct().values('integrante_id')
        ...
        if tareaIntegrante['nom_tarea'] in dontrepeYorself:
            ...
            dontrepeYorself.append(tareaIntegrante['nom_tarea'])
            ValorTotalTarea = int(
                int(tareaIntegrante['valor']))*(int(totalIntegrante['can_prod_acum']))
...
def cerrarAcumulado(request):
    ...
    try:
        ACUMULADO.objects.filter(id=idAcumulado, empresa_id=lastEm.lastEm).update(
            estatus="I", fecha_cierre=(ACUMULADO.objects.filter(id=idAcumulado).values('updated_at')))
        ProAcu.objects.filter(casino_id=idAcumulado, empresa_id=lastEm.lastEm).update(
            estatus="I", fecha_cierre=(ACUMULADO.objects.filter(id=idAcumulado).values('updated_at')))
        data = {"casino": True, "msj": "Acumulado cerrardo"}
        return Response(data)
    except Exception as e:
        data = {"casino": False,
                "msj": "Acumulado No cerrardo", "error": str(e)}
        print(data)
        return Response(data)
...
def abrirAcumulado(request):
    ...
    try:
        ACUMULADO.objects.filter(id=idAcumulado, empresa_id=lastEm.lastEm).update(
            estatus="A", fecha_cierre=(ACUMULADO.objects.filter(id=idAcumulado).values('updated_at')))
        ProAcu.objects.filter(casino_id=idAcumulado, empresa_id=lastEm.lastEm).update(
            estatus="A", fecha_cierre=(ACUMULADO.objects.filter(id=idAcumulado).values('updated_at')))
        ...
        data = {"casino": False,
                "msj": "Acumulado No abierto", "error": str(e)}
        print(data)
        return Response(data)
