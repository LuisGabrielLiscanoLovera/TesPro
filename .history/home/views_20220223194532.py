...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        lastEm.lastE   = lastEm.lastEm
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(lastEm.lastE))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastE))
