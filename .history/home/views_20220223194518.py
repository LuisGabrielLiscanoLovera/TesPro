...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        l   = lastEm.lastEm
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(l))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(l)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(l)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(l))
