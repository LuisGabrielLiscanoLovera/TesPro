...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        las   = lastEm.lastEm
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(las))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(las)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(las)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(las))
