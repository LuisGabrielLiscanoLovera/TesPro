...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(lastEm.lastEm))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.lastEm))
