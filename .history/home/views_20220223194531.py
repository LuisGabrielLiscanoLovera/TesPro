...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        lastEm.last   = lastEm.lastEm
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(lastEm.last))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.last))
