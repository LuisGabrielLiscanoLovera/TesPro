...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        lastEm.las   = lastEm.lastEm
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(lastEm.las))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm.las))
