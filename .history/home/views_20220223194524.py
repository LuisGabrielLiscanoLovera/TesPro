...
class Home(LoginRequiredMixin,TVB):
    ...
    def get_context_data(self, **kwargs):        
        ...
        RE              = Empresa.objects.filter(usuario=s['last_login'],id=int(lastEm))
        
        totalReferencia = Referencia.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
        totalColor      = Color.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
        totalIntegrante = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
        allTalla        = Talla.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
        allTarea        = Tarea.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
        totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm)).count()
        totalOperacion  = Operacion.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm)).count()
        totalCasino     = Casino.objects.all().filter(usuario=s['last_login'],empresa_id=int(lastEm))
