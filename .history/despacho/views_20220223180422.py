...
class Despacho(TemplateView):
     ...
     def get_context_data(self, **kwargs):
         ...
         #totalOperacion  = Operacion.objects.filter(empresa_id=lastEm,estatus='A').order_by('-id')
          ...
          allPatinadores   = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(idlastEmpresa),integrante_id=1)
          #allPatinador    = Integrante
          
          print(allPatinadores,3333333333333333333)
