...
class Despacho(TemplateView):
     ...
     def get_context_data(self, **kwargs):
         ...
         #totalOperacion  = Operacion.objects.filter(empresa_id=lastEm,estatus='A').order_by('-id')
          ...
          allPatinadores  = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(idlastEmpresa),id=int(patinadores[0].get('integrante_id'))).values('nombres','apellidos')
          #allPatinador    = Integrante
          
          print(allPatinadores[0].get('nombres','apellidos'),3333333333333333333)
          context = super(Despacho, self).get_context_data(**kwargs)
          
          context['allPatinador']     = patinadores      # todos los patinadores de la empresa
