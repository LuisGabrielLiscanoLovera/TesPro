...
class Despacho(TemplateView):
     ...
     def get_context_data(self, **kwargs):
         ...
         #totalOperacion  = Operacion.objects.filter(empresa_id=lastEm,estatus='A').order_by('-id')
          totalPatinador  = Patinador.objects.all().filter(usuario=s['last_login'],empresa_id=int(idlastEmpresa)).values('integrante_id')
          print(totalPatinador,3333333333333333333)
          context = super(Despacho, self).get_context_data(**kwargs)
          
          context['allPatinador']     = totalPatinador      # todos los patinadores de la empresa
