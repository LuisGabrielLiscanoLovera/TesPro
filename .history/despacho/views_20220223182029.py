...
class Despacho(TemplateView):
     ...
     def get_context_data(self, **kwargs):
         ...
         #totalOperacion  = Operacion.objects.filter(empresa_id=lastEm,estatus='A').order_by('-id')
          ...
          allPatinadores  = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(idlastEmpresa),id=1).values('nombres','apellidos')
          #allPatinador    = Integrante
          
          print(patinadores[0].get('integrante_id'),allPatinadores[0],3333333333333333333)
