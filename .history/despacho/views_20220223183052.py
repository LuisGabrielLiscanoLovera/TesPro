...
class Despacho(TemplateView):
     ...
     def get_context_data(self, **kwargs):
         ...
         #totalOperacion  = Operacion.objects.filter(empresa_id=lastEm,estatus='A').order_by('-id')
          ...
          allPatinador  = Integrante.objects.all().filter(usuario=s['last_login'],empresa_id=int(idlastEmpresa),id=int(patinadores[0].get('integrante_id'))).values('nombres','apellidos','id')
          print()
          
          allPatinadores=allPatinadores[0].get('nombres')+' '+allPatinadores[0].get('apellidos')
