...
class Integrante(models.Model):
    ...
    def __str__(self):#return f"{self.nombres}"
        return '%s %s %s %s %s %s %s %s %s %s %s %s' % (self.id, self.usuario, self.empresa,self.sexo,self.estatus,self.nombres,self.apellidos,self.correo,self.cedula,self.num_telf,self.direccion,self.abilidad,self.created_at)